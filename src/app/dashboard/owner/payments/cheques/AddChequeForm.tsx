...
export default function AddChequeForm({
  ...
  const activeContract = contracts.find(c => c.unit_id === unitId)
  const activeTenant   = activeContract ? tenants.find(t => t.id === activeContract.tenant_id) : null

  function toggleOpen() {
    if (open) { setUnitId(''); setRows([emptyRow()]) }
    setOpen(o => !o)
  }
        ...
        <form onSubmit={handleSubmit} className="px-5 pb-5 space-y-4 border-t border-slate-100 pt-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="input" required value={unitId} onChange={e => setUnitId(e.target.value)}>
            ...
            <div>
              ...
              {unitId === '' ? (
                <div className="input bg-slate-50 text-slate-400">Select a unit first</div>
              ) : activeTenant ? (
                <div className="input bg-slate-50 text-slate-700 font-medium">{activeTenant.full_name}</div>
              ) : (
                <div className="input bg-red-50 text-red-600 text-sm">No active contract on this unit</div>
              )}
            </div>
          </div>
            ...
            <>
                ...
                {rows.map((row, i) => (
                  <div key={i} className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-2 items-end">
                    ...
                    <div>
                      ...
                      <DateInput value={row.due_date} onChange={v => updateRow(i, 'due_date', v)} required />
                    </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-400">
                  {rows.length > 1 ? `${rows.length} cheques will be registered` : ''}
                </span>
