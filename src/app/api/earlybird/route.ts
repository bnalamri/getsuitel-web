...
export async function POST(req: Request) {
  const { name, email, company, role, gsm, whatsapp, country, message } = await req.json()
...
<tr><td style="background:#1B3A6B;padding:28px 32px">
  ...
  <div style="font-size:11px;color:rgba(255,255,255,0.5);letter-spacing:2px;margin-top:2px">NEW EARLY ACCESS REQUEST</div>
</td></tr>

<tr><td style="padding:28px 32px 0">
  <div style="font-size:18px;font-weight:800;color:#0f172a">Early access request from ${name}</div>
...
<tr><td style="padding:20px 32px">
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;width:160px;font-size:13px;font-weight:600;color:#64748b">Full Name</td>
    ...
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:600;color:#64748b">Company / Property</td>
      <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#0f172a">${company || '—'}</td>
    </tr>
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:600;color:#64748b">Role</td>
      <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#0f172a">${role || '—'}</td>
    </tr>
    <tr>
    ...
    <tr>
      <td style="padding:10px 0;font-size:13px;font-weight:600;color:#64748b">Country</td>
      <td style="padding:10px 0;font-size:13px;color:#0f172a">${country || '—'}</td>
    </tr>
  </table>
</td></tr>
...
<tr><td style="padding:0 32px 28px">
  <div style="background:#f8fafc;border-left:4px solid #C9931A;border-radius:0 8px 8px 0;padding:16px 20px">
...
<tr><td style="background:#f8fafc;padding:16px 32px;border-top:1px solid #e2e8f0">
  <div style="font-size:11px;color:#94a3b8">Sent via getsuitel.com early access form · ${new Date().toUTCString()}</div>
</td></tr>
  ...
  try {
    const { error } = await resend.emails.send({
      from: 'GetSuitel Early Access <notices@getsuitel.com>',
      to: ['earlybirds@omanportal.net'],
      replyTo: email,
      subject: `Early access request from ${name}${country ? ` (${country})` : ''}`,
      html,
    })
